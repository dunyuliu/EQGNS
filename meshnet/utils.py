...
def datas_to_graph(training_example, dt, device):
    ...
    node_property = training_example[0][2].to(device)  # (nnodes, 1)
    velocity_feature = training_example[0][3].to(device)  # (nnodes, dims)
    pressure = training_example[0][4].to(device)  # (nnodes, 1)
    cells = training_example[0][5].to(device)  # (ncells, nnodes_per_cell)
    cells = torch.transpose(cells, 0, 1).type(torch.LongTensor).to(device)
    time_vector = training_example[0][6] * dt  # (nnodes, )
    time_vector = time_vector.unsqueeze(1).to(device)
    # n_node_per_example = training_example[0][6]

    # aggregate node features
    node_features = torch.hstack((node_type, node_property, velocity_feature, pressure, time_vector)).to(device)
